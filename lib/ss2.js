...
var SS2 = (function(){
  ...
  var anonyUser = {
    uname   : 'anonymous',
    uid     : 'anonymous',
    avatar  : 'https://fbcdn-profile-a.akamaihd.net/static-ak/rsrc.php/v1/yo/r/UlIqmHJn-SK.gif',
    uType   : 'null',
    isAdmin : false
  };
  ...
  ///*** Is it util? ***///
    ...
    getUser : function(req){
      return __s.isEmptyObject(req.session.user)?anonyUser:req.session.user;
    },

    updateUser : function(req, data){
      var user = __s.getUser(req);
      for ( var name in data ) {
        if(user[name] !== undefined && data[name] !== undefined){
          user[name] = data[name];
        }
      }
      req.session.user = user;
    ...
    isLogin : function(req){
      return !__s.isEmptyObject(__s.getUser(req)) && !__s.isGuest(req);
    },
    
    isAdmin : function(req){
      return __s.isLogin(req)?__s.getUser(req).isAdmin:false;
    },

    isGuest : function(req){
      return (__s.getUser(req) === anonyUser);
    }
  };
